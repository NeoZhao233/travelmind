...
- 实现 BM25 与 BGE 中文向量检索并以 RRF 融合；构建 105 条 Query、35 个意图簇的分层
  评测草案，扩集后 Hybrid Recall@5 为 0.971；Cross-Encoder 降至 0.958 且平均本地延迟
...
- 构建 105 Query/35 意图簇的分层检索草案；Hybrid Recall@5 为 0.971，并拒绝使其降至
...
- Do not present the Stage 12 retrieval numbers as reviewed benchmark results until a person validates
  the 35 intent clusters; generated structure is not independent annotation.

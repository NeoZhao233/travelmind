...
4 个受控案例中可恢复故障恢复率从固定计划的 0% 提升到 100%。检索使用 BM25+BGE+RRF，
种子集 Recall@5 为 0.956；Reranker 和无增量价值的 DeepSeek Planner 均被实验门禁拒绝。
...
已经成功的检索结果。4 个多步故障案例中，两类可恢复中途失败都完成，双路径失败则不生成
无证据行程。这个结果证明的是受控的恢复契约，不是线上 API 可用性。

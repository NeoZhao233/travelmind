...
我做了一个基于 LangGraph 的旅游规划 Agent Harness，重点不是前端，而是 Agentic RAG、上下文工程、
...
工程上还用官方 MCP SDK 标准化四类工具，通过 Allowlist 和 Tool Policy 限制缓存与降级，
并实现了可选 Redis Cache/Checkpoint、有界重试、幂等、索引原子发布和安全失败。
...
整体上我把它定义为 Agent Harness，而不是把 RAG 当作整个系统：RAG 是知识工具，Harness
负责模型/工具循环、上下文、权限、预算、状态恢复、错误归因和终止。四类运行时工具通过官方
MCP v2 SDK 暴露，但 MCP 放在受控 Registry 后；只有传输断连且工具只读幂等时才允许本地
降级，远端业务失败仍会进入 Observation。Redis 只作为可选 TTL Cache 与共享 Checkpoint，
不会为了堆栈替换已有 Qdrant；缓存失败可绕过，Checkpoint 失败则关闭，避免执行历史分叉。
...
会把小样本结果称为生产指标。最终由 v3 发布清单固定历史证据、Agentic Runtime 与 Harness
证据，并自动校验哈希和语义声明。
...
最后我建立 v3 发布清单，将 29 份关键代码锁、架构决策和实验报告做 SHA-256 固定，再验证
33 项语义声明，共 62 项审计全部通过。这保证我面试里说的每个数字都能落到版本化证据上。

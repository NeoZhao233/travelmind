...
约束规划和评测闭环。检索使用 BM25+BGE+RRF，种子集 Recall@5 达到 0.956；上下文流水线
把 Token 降低 45.3%。我没有默认保留所有模型组件：Reranker 导致 Recall 下降，DeepSeek
Planner 增加 4,475 Token 却没有质量提升，所以都被实验门禁拒绝。工程上还实现了有界重试、
Checkpoint、幂等、熔断缓存、索引原子发布和安全失败，最后用 33 项发布审计固定证据边界。
...
项目的另一个重点是评测驱动选择。Hybrid RRF 在 15 条种子 Query 上 Recall@5 为 0.956。
加入 Cross-Encoder 后反而降到 0.900，我分析是小候选集、领域错配和排序目标不一致造成相关
文档被挤出 top-k，因此保留接口但不进入默认链路。上下文方面，选出的 768-token pipeline 在
...
会把小样本结果称为生产指标。最终 15 份证据被哈希固定，并通过 33 项自动审计。
...
不可比分数。在 15 条种子 Query 上，Recall@5、MRR@5、NDCG@5 分别是 0.956、0.956、
0.933。Cross-Encoder 消融把 Recall@5 降到 0.900，因此没有因为它“更高级”就上线。
...
最后我建立发布清单，将 15 份关键代码锁、架构决策和实验报告做 SHA-256 固定，再验证 18 项
语义声明，共 33 项审计全部通过。这保证我面试里说的每个数字都能落到版本化证据上。
...
例如回答“为什么不默认使用 Reranker”：目标是改善 Hybrid 候选的 top-k 顺序；但固定候选消融
中 Recall@5 从 0.956 降到 0.900，因此按预先定义的门禁拒绝。可能原因包括通用模型领域错配、
